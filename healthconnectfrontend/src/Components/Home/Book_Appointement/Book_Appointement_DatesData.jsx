...
const Book_Appointement_DatesData = () => {
  ...
  function bookHandler(){
    ...
    const bookingHospital = async ()=>{
      setLoading(true)
      const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/book-appointment`,{
        method:"POST",
        headers:{
          'content-type':"application/json"
        },
                  ...
                  <Calendar
                    onClickDay={(val) => {
                      setAppointmentData((appointmentData) => {
                        console.log(val.getFullYear()+'-'+val.getDate()+'-'+val.getMonth());
                        const formattedDate = `${val.getFullYear()}-${(val.getMonth() + 1).toString().padStart(2, '0')}-${val.getDate().toString().padStart(2, '0')}`;
